-- Ajoute les livrables cabinet dans firm_inbox
-- + rafraîchit fill_firm_inbox
...
BEGIN
  ...
  -- 2. Documents déposés par clients (source = customer)
  INSERT INTO firm_inbox (firm_id, event_type, source_type, source_id, customer_id, payload, event_at)
  SELECT
    d.firm_id,
    'document_uploaded',
  ...
  -- 3. Livrables déposés par le cabinet (source = firm)
  INSERT INTO firm_inbox (firm_id, event_type, source_type, source_id, customer_id, payload, event_at)
  SELECT
    d.firm_id,
    'livrable_uploaded',
    ...
    d.customer_id,
    jsonb_build_object(
      'filename',      COALESCE(d.filename, 'Livrable sans nom'),
      'customer_name', c.name,
      'doc_type',      COALESCE(dt.name, '')
    ),
    de.created_at
...
-- Repopulation avec les nouveaux types
