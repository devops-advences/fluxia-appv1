...
import {
  type LoadedFile, type CabQualification, type DocType, type FileKind,
  MONTHS, DOC_COLORS, FILE_KIND_META,
...
type Props = {
  ...
  onQualChange:   (key: string, q: CabQualification) => void
  onAllDone:      () => void
}

export default function Step2QualifyLivrable({
  files, cuts, skips, quals, clientCountry,
  curFi, curDi, previewP,
  ...
  useEffect(() => {
    supabase.from('document_type').select('id, name')
      .eq('customer', false).eq('active', true).eq('country_code', clientCountry).order('rank')
  ...
  const q          = quals[key] ?? { typeId: '', typeName: '', year: new Date().getFullYear().toString(), month: '', note: '' }
  const allCommitted = allDocs.length > 0 && allDocs.every(d => !!quals[qualKey(d.fi, d.di)]?.committed)

  const setField = useCallback(<K extends keyof CabQualification>(field: K, value: CabQualification[K]) => {
    onQualChange(key, { ...q, [field]: value })
  }, [key, q, onQualChange])

  const setType = (id: string) => {
    const t = docTypes.find(t => t.id === id)
    onQualChange(key, { ...q, typeId: id, typeName: t?.name ?? '' })
  }
  ...
  return (
    <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ width: '195px', flexShrink: 0, borderRight: '1px solid #e5e7eb', overflowY: 'auto', background: '#f9fafb' }}>
        {noCutsYet && hasPdfs && (
          <div style={{ margin: '8px 8px 0', padding: '7px 9px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '6px', fontSize: '11px', color: '#92400e', lineHeight: 1.4 }}>
            ✂ Cliquez <strong>entre deux pages</strong> pour séparer en plusieurs livrables
          </div>
        )}
        {files.map((file, fi) => {
          const docs = getDocs(file.pageCount, cuts[fi] ?? new Set())
          return (
            <div key={file.id}>
              <div style={{ padding: '6px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {docs.map((doc, di) => {
                  ...
                  const docLabel  = `Livrable ${globalIdx + 1}`
                      ...
                      <div key={di}>
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '11px', fontWeight: 600, color: '#065f46', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{qual.typeName || '—'}</div>
                            <div style={{ fontSize: '10px', color: '#059669', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{qual.typeName}</div>
                          </div>
                          {file.fileKind === 'pdf' && (
                        ...
                        {!isLastDoc && file.fileKind === 'pdf' && (
                          <CutZone hasCut={(cuts[fi] ?? new Set()).has(doc.end)} onClick={() => onCutToggle(fi, doc.end)} />
                        )}
                      </div>
                    )
                  }
                      ...
                      <div key={di}>
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <span style={{ fontSize: '10px', color: '#f59e0b' }}>Non qualifié</span>
                          </div>
                        </div>
                      </div>
                    )
                    ...
                    <div key={di} style={{ marginTop: di > 0 ? '2px' : 0 }}>
                      <div onClick={() => onSelectPage(fi, di, doc.start)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 5px 3px', cursor: 'pointer' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: '10px', color: '#f59e0b', marginLeft: '5px' }}>Non qualifié</span>
                        </div>
                      </div>
                      {Array.from({ length: doc.end - doc.start + 1 }, (_, i) => doc.start + i).map(p => {
                        const isPrev = isActive && p === safePreviewP
                        const isSkip = (skips[fi] ?? new Set()).has(p)
                        const hasCut = (cuts[fi] ?? new Set()).has(p)
                        return (
                          <div key={p}>
                            <div
                              ...
                              style={{
                                ...
                                opacity: isSkip ? 0.5 : 1, transition: 'all 0.1s', position: 'relative',
                              }}
                            >
                              ...
                              <button
                                ...
                                title={isSkip ? 'Restaurer' : 'Ignorer cette page'}
      ...
      <div style={{ width: '250px', flexShrink: 0, borderLeft: '1px solid #e5e7eb', background: '#fff', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
          <div style={{ fontSize: '12px', fontWeight: 600, color: '#111827' }}>Livrable {gi + 1} / {allDocs.length}</div>
        ...
        {allCommitted && !editing && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', gap: '10px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '13px', fontWeight: 600, color: '#059669' }}>Tous les livrables<br />sont qualifiés</div>
            <div style={{ fontSize: '11px', color: '#9ca3af', lineHeight: 1.5 }}>Cliquez sur un livrable<br />à gauche pour le modifier</div>
          </div>
        )}
        ...
        {(!allCommitted || editing) && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <FormGroup label="Type de livrable">
              <select value={q.typeId} onChange={e => setType(e.target.value)} style={selectStyle}>
            ...
            <FormGroup label="Note">
              <textarea
                ...
                rows={3}
                style={{ ...selectStyle, resize: 'none', backgroundImage: 'none', paddingRight: '10px', height: '70px' }}
              />
            </FormGroup>

            {(() => {
              const canValidate = !!q.year
...
function CutZone({ hasCut, onClick }: { hasCut: boolean; onClick: () => void }) {
  return (
    <div onClick={onClick} className={`cut-zone ${hasCut ? 'has-cut' : ''}`}
      style={{ height: '20px', display: 'flex', alignItems: 'center', margin: '0 3px', position: 'relative', cursor: 'pointer' }}>
      ...
      {!hasCut && <span className="cut-hint" style={{ position: 'relative', zIndex: 1, fontSize: '10px', color: '#9ca3af', opacity: 0.35, transition: 'opacity 0.12s', pointerEvents: 'none' }}>✂</span>}
    </div>
  )
}
...
const btnBase:    React.CSSProperties = { padding: '7px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 500, cursor: 'pointer', border: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px', fontFamily: 'inherit', lineHeight: 1.4, transition: 'all 0.12s' }
